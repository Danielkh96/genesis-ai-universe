...
// In zoom mode: camera locks 4 units BEHIND the planet (between planet and camera origin point)
// Planet always stays centered. Camera follows planet orbit indefinitely.
function CameraController({ zoomAgentId }: { zoomAgentId: string | null }) {
  ...
  useFrame((_, delta) => {
    if (zoomAgentId) {
      ...
      if (planetPos && planetPos.lengthSq() > 0.1) {
        // outward = direction from scene origin toward the planet
        const outward = planetPos.clone().normalize();
        // Camera sits BEYOND the planet (further from origin than the planet)
        // at distance 4 units from planet surface, looking back AT the planet
        // This guarantees planet is dead-center in frame at all times
        const desiredPos = planetPos.clone().add(outward.clone().multiplyScalar(4));
        // Very fast lerp to stay glued to the moving planet
        const sp = Math.min(1, delta * 10);
        pos.current.lerp(desiredPos, sp);
        look.current.lerp(planetPos, sp);
...
varying vec3 vNormal; varying vec3 vPosition; varying vec2 vUv;
float hexGrid(vec2 p,float scale){p*=scale;vec2 r=vec2(1.0,1.732);vec2 h=r*0.5;vec2 a=mod(p,r)-h;vec2 b=mod(p-h,r)-h;vec2 gv=dot(a,a)<dot(b,b)?a:b;return smoothstep(0.44,0.46,length(gv));}
void main(){
...
function AgentPlanet3D({agent,active,onClick}:{agent:Agent;active:boolean;onClick:()=>void}){
  ...
  const moons=useRef<THREE.Group>(null); const angle=useRef(agent.orbitAngle);
  ...
  useFrame(({clock},delta)=>{
    ...
    if(group.current){
      group.current.position.set(Math.cos(angle.current)*r,Math.sin(angle.current*1.3)*r*0.18,Math.sin(angle.current)*r);
...
export default function GenesisUniverse(){
  ...
  useEffect(()=>{
    const fn=(e:TouchEvent)=>{const t=e.touches[0];if(!t)return;setTouchGlow({x:(t.clientX/window.innerWidth)*100,y:(t.clientY/window.innerHeight)*100,active:e.type!=="touchend"});};
    window.addEventListener("touchstart",fn,{passive:true});window.addEventListener("touchmove",fn,{passive:true});window.addEventListener("touchend",fn);
  ...
  const zoomToAgent=useCallback((id:string)=>{
    const agent=agents.find(a=>a.id===id);if(!agent)return;
    setActiveId(id); setZoomAgentId(id); setBurst({id,label:agent.name});
  },[]);
  const resetCamera=useCallback(()=>{
    setZoomAgentId(null); setActiveId("automation"); setBurst({id:"core",label:"Genesis Core"});
  },[]);
  ...
  return(
    <main className={`portal-shell ${webglReady?"webgl-ready":""}`} style={{["--mx" as string]:`${mouse.x}%`,["--my" as string]:`${mouse.y}%`,["--active" as string]:active.color}}>
      <div className="portal-viewport">
        ...
        {webglReady&&(
          <Canvas className="portal-canvas" camera={{position:[0,1.2,42],fov:52,near:0.05,far:1200}} dpr={[1,1.8]}
        ...
        <section className="galaxy-stage" aria-label="Genesis AI Universe portal">
          {!webglReady&&(<><div className="orbit-ring orbit-ring-1"/><div className="orbit-ring orbit-ring-2"/><div className="orbit-ring orbit-ring-3"/><button className="central-brain" aria-label="Genesis central AI brain"><span className="brain-mesh"/><span className="brain-core-glow"/><BrainCircuit className="brain-icon"/><span className="brain-label">GENESIS<br/>CORE</span></button></>)}
          {agents.map(agent=>{const Icon=agent.icon;const sel=activeId===agent.id;return(<button key={agent.id} data-module-id={agent.id} onClick={()=>zoomToAgent(agent.id)} className={`agent-planet ${sel?"selected":""}`} style={{left:`${agent.hud.x}%`,top:`${agent.hud.y}%`,["--node" as string]:agent.color,["--node-glow" as string]:agent.glow}}><span className="planet-orbit"/><span className="planet-body"><Icon className="h-7 w-7"/></span><span className="planet-label"><strong>{agent.name}</strong><em>{agent.role}</em></span></button>);})}
        ...
        <AnimatePresence mode="wait">
          <motion.div key={burst.id} className="cinematic-burst" initial={{opacity:0,scale:0.72,filter:"blur(18px)"}} animate={{opacity:[0,1,0.16],scale:[0.72,1.2,1],filter:["blur(18px)","blur(0px)","blur(2px)"]}} exit={{opacity:0}} transition={{duration:1.1,ease:"easeOut"}}>{burst.label}</motion.div>
        </AnimatePresence>
        <AnimatePresence mode="wait">
      ...
      <div className="scroll-sectors" aria-hidden="true">
        {[{id:"genesis-core",label:"Genesis Core"},...agents.map(a=>({id:`ai-${a.id}`,label:a.name}))].map(({id,label},idx)=>(<section key={id} id={id} className="scroll-sector"><div><span>{String(idx).padStart(2,"0")}</span><strong>{label}</strong></div></section>))}
      </div>
    </main>
  );
}