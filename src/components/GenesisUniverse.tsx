...
// ── CameraController ──────────────────────────────────────────────────────────
// TRUE CLOSE-UP TRACKING:
// - Camera placed 5 units BEHIND planet (between origin and planet)
// - lookAt(planet) every frame → brain at origin is BEHIND camera (not visible)  
...
// Direction from origin to planet
const outward = p.clone().normalize();
// Small upward perp component for slight elevation
const worldUp = new THREE.Vector3(0, 1, 0);
const upPerp = worldUp.clone().sub(outward.clone().multiplyScalar(worldUp.dot(outward))).normalize();
// Camera positioned 5 units BEHIND the planet (between origin and planet)
// So the brain at origin is BEHIND the camera and not visible
const desired = p.clone().sub(outward.clone().multiplyScalar(5)).add(upPerp.clone().multiplyScalar(1.2));
...
// Look at planet directly - planet always perfectly centered
cam.lookAt(p.x, p.y, p.z);
cam.updateMatrixWorld(true);
// Narrow FOV = telephoto lens zoom into planet
...
float hexGrid(vec2 p, float scale) {
  ...
  return smoothstep(0.44, 0.46, length(gv));
}
void main() {
...
function CentralBrain3D({ color }: { color: string }) {
  ...
  const o0 = useRef<THREE.Group>(null); const o1 = useRef<THREE.Group>(null);
  const o2 = useRef<THREE.Group>(null); const o3 = useRef<THREE.Group>(null);
  ...
  useFrame(({ clock }) => {
    ...
    if (o3.current) o3.current.rotation.z = -t * 0.38;
  ...
  return (
    <group ref={brain}>
      ...
      <group ref={o0} rotation={[0.66, 0.08, 0.15]}>
        ...
        <mesh position={[4.05, 0, 0]}><sphereGeometry args={[0.095, 22, 22]} /><meshBasicMaterial color="#fff" transparent opacity={1} blending={THREE.AdditiveBlending} /></mesh>
      </group>
      <group ref={o1} rotation={[1.15, 0.5, 0.8]}>
      ...
      <group ref={o2} rotation={[0.2, 1.28, 0.35]}>
        ...
        <mesh position={[0, 6.35, 0]}><sphereGeometry args={[0.06, 18, 18]} /><meshBasicMaterial color="#fff" transparent opacity={0.9} blending={THREE.AdditiveBlending} /></mesh>
      </group>
      <group ref={o3} rotation={[1.35, -0.55, 0.1]}>
        <mesh><torusGeometry args={[7.35, 0.01, 18, 420]} /><meshBasicMaterial color="#fff" transparent opacity={0.33} blending={THREE.AdditiveBlending} depthWrite={false} /></mesh>
      </group>
      <group ref={sparks}>
        {Array.from({ length: 42 }, (_, i) => {
          const a = (i / 72) * Math.PI * 2; const r = 3.05 + (i % 7) * 0.48;
          return (<mesh key={i} position={[Math.cos(a) * r, Math.sin(a * 1.4) * 1.85, Math.sin(a) * r * 0.62]}>
            <sphereGeometry args={[0.025 + (i % 4) * 0.013, 10, 10]} />
            <meshBasicMaterial color={i % 3 === 0 ? "#fff" : i % 3 === 1 ? "#67e8f9" : color} transparent opacity={0.95} blending={THREE.AdditiveBlending} />
          </mesh>);
        })}
      </group>
      <pointLight color="#fff" intensity={28} distance={26} />
    ...
    <div style={{ pointerEvents: "none", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'Geist',Arial,sans-serif" }}>
      <div style={{ display: "inline-block", position: "relative", background: active ? "linear-gradient(135deg,rgba(2,8,23,.92),rgba(8,16,36,.88))" : "rgba(2,8,23,.72)", border: `1px solid ${color}${active ? "cc" : "55"}`, borderRadius: 10, padding: active ? "7px 12px" : "5px 9px", backdropFilter: "blur(14px)", boxShadow: active ? `0 0 24px ${color}60,0 0 48px ${color}28` : `0 0 14px ${color}30`, transition: "all .3s" }}>
        ...
        {active && (<div style={{ marginTop: 5, paddingTop: 4, borderTop: `1px solid ${color}40`, display: "flex", alignItems: "center", gap: 5, justifyContent: "center" }}>
          ...
          <span style={{ color, fontSize: 8, fontWeight: 800, letterSpacing: ".2em" }}>ACTIVE</span>
        </div>)}
      </div>
...
// ── AgentPlanet3D ─────────────────────────────────────────────────────────────
// Key: writes world position to planetWorldPositions every frame via getWorldPosition()
function AgentPlanet3D({ agent, active, onClick }: { agent: Agent; active: boolean; onClick: () => void }) {
  ...
  const angle = useRef(agent.orbitAngle);
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (group.current) {
      group.current.position.set(
        ...
        Math.sin(angle.current * 1.3) * r * 0.18,
        Math.sin(angle.current) * r
      );
      group.current.rotation.y = t * 0.22;
      // Publish real world position for CameraController
      group.current.getWorldPosition(wp.current);
      planetWorldPositions.set(agent.id, wp.current.clone());
    }
    ...
    <group ref={group}>
      ...
      <group ref={moons} rotation={[0.45, 0.15, 0.2]}>
        {Array.from({ length: 4 }, (_, i) => {
          ...
          return (<group key={i} rotation={[i * 0.32, a, i * 0.18]}>
            <mesh position={[Math.cos(a) * mr, Math.sin(a * 1.6) * 0.34, Math.sin(a) * mr]}>
              <sphereGeometry args={[active ? 0.13 : 0.09, 18, 18]} />
              <meshBasicMaterial color={i % 2 === 0 ? "#fff" : agent.color} transparent opacity={0.92} blending={THREE.AdditiveBlending} depthWrite={false} />
            </mesh>
          </group>);
        })}
      </group>
...
function Galaxy({ galaxyIndex, agents: ga, activeId, onPlanetClick }: { galaxyIndex: number; agents: Agent[]; activeId: string; onPlanetClick: (id: string) => void }) {
  ...
  const tilt = ga[0]?.orbitTilt ?? [0, 0, 0];
...
function BackgroundParticles() {
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    if (g.current) {
      (g.current.children[0] as THREE.Points).rotation.y = t * 0.012;
      (g.current.children[1] as THREE.Points).rotation.y = -t * 0.008;
      (g.current.children[2] as THREE.Points).rotation.x = t * 0.005;
    }
  });
  return (<group ref={g}>
    <points geometry={geo1}><pointsMaterial size={1.8} color="#fff" transparent opacity={0.65} sizeAttenuation /></points>
...
function FlyingGlowPlanets() {
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    if (g.current) g.current.children.forEach((c, i) => { const f = fl[i]; if (!f) return; const tr = ((t * f.speed + f.phase) % 1) * 2 - 1; c.position.set(tr * f.spread, f.y + Math.sin(t * 0.8 + f.phase) * 0.35, f.z + Math.cos(t * 0.45 + f.phase) * 0.8); });
  });
...
function NetworkNodes() {
  const g = useRef<THREE.Group>(null);
  const { nodes, lp } = useMemo(() => {
    const nodes = Array.from({ length: 160 }, (_, i) => { const a = (i * 2.399963) % (Math.PI * 2), r = 18 + ((i * 17) % 100) / 100 * 68, y = Math.sin(i * 1.37) * 22; return { p: new THREE.Vector3(Math.cos(a) * r, y, Math.sin(a) * r * 0.9 - 16), s: 0.018 + (i % 5) * 0.008, color: i % 5 === 0 ? "#fb7185" : i % 5 === 1 ? "#fbbf24" : i % 5 === 2 ? "#a78bfa" : "#67e8f9" }; });
    const v: number[] = []; nodes.forEach((n, i) => { const nx = nodes[(i + 13) % nodes.length]!; if (n.p.distanceTo(nx.p) < 22 || i % 7 === 0) v.push(n.p.x, n.p.y, n.p.z, nx.p.x, nx.p.y, nx.p.z); });
    return { nodes, lp: new Float32Array(v) };
  }, []);
...
export default function GenesisUniverse() {
  ...
  // Camera will continuously follow this planet until reset or another agent clicked.
  const zoomToAgent = useCallback((id: string) => {
    const agent = agents.find(a => a.id === id); if (!agent) return;
    setActiveId(id);
    setZoomAgentId(id); // camera tracks this agent indefinitely
    setBurst({ id, label: agent.name });
  }, []);

  const resetCamera = useCallback(() => {
    setZoomAgentId(null); // stop tracking, return to free orbit
      ...
      <div className="portal-viewport">
        ...
        <section className="galaxy-stage" aria-label="Genesis AI Universe portal">
          ...
          {navPlanets.map((planet, idx) => (
            <a key={planet.id} className="nav-asteroid" href={planet.url}
              style={{ left: `${planet.hud.x}%`, top: `${planet.hud.y}%`, ["--node" as string]: planet.color, animationDelay: `${idx * -0.4}s` }}>
