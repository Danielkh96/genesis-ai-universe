...
// When zoomAgentId is set, camera stays ~4 units away from the planet,
// positioned along the outward direction from origin through planet.
// camera.lookAt(planet) called directly every frame - NO lerp on look.
// Only position is lerped (fast) to smoothly follow the moving planet.
...
// Camera offset: outward from origin through planet, distance = 4 units
const outward = p.clone().normalize();
// Add slight upward tilt so we see the planet rings nicely
const up = new THREE.Vector3(0, 1, 0);
// Gram-Schmidt: make up perpendicular to outward
const upPerp = up.clone().sub(outward.clone().multiplyScalar(up.dot(outward))).normalize();
// Position = planet + outward * 4 + upPerp * 0.8 (slight elevation)
const desired = p.clone()
.add(outward.clone().multiplyScalar(4))
.add(upPerp.clone().multiplyScalar(0.8));
// Fast lerp on position so it snaps quickly and tracks the orbiting planet
const speed = Math.min(1, delta * 10);
pos.current.lerp(desired, speed);
camera.position.copy(pos.current);
// Direct lookAt - NO lerp, planet always perfectly centered
